...
from winner1.code import wrappers


def get_initializer(name):
    from winner1.code.env import initializers
    if name is None:
        return None
...
def get_reward_fn(name):
    from winner1.code.env import reward_fns
    if name is None:
...
def get_termination_fn(name):
    from winner1.code.env import termination_fns
    if name is None:
        return None
...
def make_training_env(cube_goal_pose, goal_difficulty, action_space, frameskip=1,
    ...
    # changed step_size from 100 to 10
    env = SimCubeTrajectoryEnv(goal_trajectory=None,
                               frameskip=frameskip,
    ...
    #     from winner1.code.const import TMP_VIDEO_DIR
    #     env = Monitor(
